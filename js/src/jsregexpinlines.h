...
#ifdef TAINTED
TAINT_COND_SET_NEW(captured,strArg,NULL,REGEXP)             
#endif
...
RegExpStatics::createDependent(JSContext *cx, size_t start, size_t end, Value *out) const 
{
    ...
    TAINT_COND_SET_NEW(str , matchPairsInput,NULL,REGEXP)
...
RegExpStatics::makeMatch(JSContext *cx, size_t checkValidIndex, size_t pairNum, Value *out) const
{
    if (checkValidIndex / 2 >= pairCount() || matchPairs[checkValidIndex] < 0) {
        ...
        if( matchPairsInput->isTainted()){
         JSString *rstr= taint_newTaintedString(cx,cx->runtime->emptyString ); 
       
         addTaintInfoOneArg(cx, matchPairsInput ,rstr,NULL,REGEXP ); 
         out->setString( rstr );
         return JS_TRUE;         
        }
#endif    
